...
//======================================================================
std::string Animator::getVehicleColor(Section* sptr)
{
    Direction dir = sptr->getSectionDirection();
    switch (sptr->getVehicleType())
    {
...
void Animator::drawNorthPortion(int time)
{
    for (int s = 0; s < numSectionsBefore; s++)
    {
        ...
        if (southToNorth[section]->isOccupied()==false)
            std::cout << Animator::EMPTY_SECTION;
        else
...
void Animator::drawEastbound()
{
    ...
    {
        int section = s;
        if (westToEast[section]->isOccupied()==false)
            std::cout << Animator::EMPTY_SECTION << "|";
        else
    ...
    Section* sptr = (westToEast[numSectionsBefore]->isOccupied()==false ?
            westToEast[numSectionsBefore] : northToSouth[numSectionsBefore + 1]);
    if (sptr->isOccupied()==false)
        std::cout << Animator::EMPTY_SECTION << "|";
    else
        std::cout << getVehicleColor(sptr)
                  << std::setfill('0') << std::setw(Animator::DIGITS_TO_DRAW)
                  << sptr->getVehicleID()
                  << Animator::COLOR_RESET << "|";
    ...
    sptr = (westToEast[numSectionsBefore + 1] != nullptr ?
...
void Animator::drawWestbound()
{
    ...
    for (int s = 0; s < numSectionsBefore; s++)
    {
        ...
        if (eastToWest[section]->isOccupied()==false)
            std::cout << Animator::EMPTY_SECTION << "|";
        else
    ...
    // the N2S lane
    Section* sptr = (eastToWest[numSectionsBefore + 1]->isOccupied()==false ?
    ...
    sptr = (eastToWest[numSectionsBefore] != nullptr ?
...
void Animator::drawSouthPortion()
{
    for (int s = 0; s < numSectionsBefore; s++)
    {
        ...
        if (northToSouth[section]->isOccupied() == false)
            std::cout << Animator::EMPTY_SECTION;
        else
